...
public class DataBase implements Serializable {
	...
	public synchronized int chunksDataSize() {
		int i = 0;
	...
	public synchronized void addChunkRemovedPutChunk(int no, String fileID) {
		Chunk chunk = new Chunk(no, fileID);
		if (!removedPutChunks.containsKey(chunk))
			this.removedPutChunks.put(chunk, new Boolean(false));
	}

	public synchronized void setTrueChunkRemovedPutChunk(int no, String fileID) {
		Chunk chunk = new Chunk(no, fileID);
		if (removedPutChunks.containsKey(chunk))
			this.removedPutChunks.put(chunk, new Boolean(true));
	}

	public synchronized boolean getChunkRemovedPutChunk(int no, String fileID) {
		Chunk chunk = new Chunk(no, fileID);
		return this.removedPutChunks.get(chunk);
	}

	public synchronized void removeChunkRemovedPutChunk(int no, String fileID) {
		Chunk chunk = new Chunk(no, fileID);
