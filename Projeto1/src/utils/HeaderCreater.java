...
public class HeaderCreater extends MessageType {
	...
	public static byte[] putChunk(String fileId, int chunkNo, int replicationDeg) {
		String Header = PUTCHUNK + SPACE + Peer.getVersion() + SPACE + Peer.getPeerID() + SPACE + fileId + SPACE
	...
	public static byte[] stored(String fileId, String chunkNo) {
		String Header = STORED + SPACE + Peer.getVersion() + SPACE + Peer.getPeerID() + SPACE + fileId + SPACE
	...
	public static byte[] getChunk(String fileId, int chunkNo) {
		String Header = GETCHUNK + SPACE + Peer.getVersion() + SPACE + Peer.getPeerID() + SPACE + fileId + SPACE
				+ chunkNo + SPACE + CRLF + CRLF;
	...
	public static byte[] chunk(String fileId, int chunkNo) {
		String Header = CHUNK + SPACE + Peer.getVersion() + SPACE + Peer.getPeerID() + SPACE + fileId + SPACE
	...
	public static byte[] delete(String fileId) {
		String Header = DELETE + SPACE + Peer.getVersion() + SPACE + Peer.getPeerID() + SPACE + fileId + SPACE + CRLF + CRLF;

		return Header.getBytes(Charset.forName("ISO_8859_1"));
	}
	
	public static byte[] removed(String fileId, int chunkNo) {
		String Header = REMOVED + SPACE + Peer.getVersion() + SPACE + Peer.getPeerID() + SPACE + fileId + SPACE
				+ Integer.toString(chunkNo) + SPACE + CRLF + CRLF;
